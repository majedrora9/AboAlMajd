...
async function fetchAvailableTimes() {
    try {
        ...
        if (data.availableTimes && Array.isArray(data.availableTimes)) {
            ...
            bookButton.disabled = true; // يتم تفعيل زر الحجز عند اختيار الوقت
        } else {
...
function isMonday() {
    ...
    return today.getDay() === 2; // 1 يمثل يوم الاثنين (الأحد هو 0)
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    bookButton.addEventListener('click', async () => {
        ...
        // التحقق إذا كان المستخدم قد حجز بالفعل في نفس اليوم
        try {
        ...
        // إرسال طلب الحجز إلى Google Apps Script
        try {
